...
import { divArrays } from "../../utils/divArrays";
import { indexOfMax } from "../../utils/indexOfMax";
import { replace } from "../../utils/replace";
import { sliceColumn } from "../../utils/sliceColumn";
import { subArrays } from "../../utils/subArrays";
import { swapColumns } from "../../utils/swapColumns";
import { swapRows } from "../../utils/swapRows";
 ...
 */
export function LUPdecomp(a: number[][], tol: number = 1.0e-9): [number[][], number[]] {
  const n: number = a.length;
  let seq: number[] = Array.from(Array(n).keys());
  ...
  for (let k = 0; k < n - 1; k++) {
    ...
    if (Math.abs(a[p][k]) < tol) throw Error("Matrix is singular");
    if (p !== k) {
      swapColumns(s, k, p);
      swapRows(a, k, p);
      swapColumns(seq, k, p);
